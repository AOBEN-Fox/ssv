...
grep -q 'helmet-event-enabled' scripts/pipeline.sh || fail "pipeline does not pass helmet event config to ssvpub"
grep -q 'SSV_EVIDENCE_OUTPUT_DIR' scripts/pipeline.sh || fail "pipeline does not expose evidence output dir"
grep -q 'RIGHT_CODES_API_KEY' README.md || fail "README does not document Right Codes API key"
